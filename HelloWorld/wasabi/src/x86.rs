...
impl<const LEVEL: usize, NEXT> Entry<LEVEL, NEXT> {
    ...
    fn format(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            ...
            self,
            self.read_value(),
            if self.is_present() { "P" } else { "p" },
            if self.is_writable() { "W" } else { "w" },
...
// # Safety
// Anyting can happend if the given selector is invalid.
...
// # Safety
// Anyting can happen if the CS give is invalid.
...
// # Safety
// Anyting can happen if the give selector is invalid.
...
// # Safety
// Anyting can happen if the given selector is invalid.
...
macro_rules! interrupt_entrypoint {
    ($index:literal) => {
        global_asm!(concat!(
            ...
            "push rcx // Save the context pointer\n",
            "mov rcx, ",
            stringify!($index),
            "\n",
...
macro_rules! interrupt_entrypoint_with_ecode {
    ($index:literal) => {
        global_asm!(concat!(
            ...
            stringify!($index),
            ":\n",
            "push rcx // Saven rcx first to reuse\n",
            "mov rcx, ",
            stringify!($index),
            "\n",
            "jmp inthandler_common"
...
impl Drop for TaskStateSegment64 {
    fn drop(&mut self) {
        panic!("TSS64 dropped, this is not allowed!");
    }
}
...
pub const BIT_DPL0: u64 = 0x64 << 45;
pub const BIT_DPL3: u64 = 0x67 << 45;
...
// # Safety
// Writing to CR3 can causes any exceptions os it is programmer's resposibility to setup correct page tables.
#[no_mangle]
