...
impl<T: Sized> Debug for Mutex<T> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "Mutex @ {}:{}", self.created_at_file, self.created_at_line)
    }
}
impl<T: Sized> Mutex<T> {
    ...
    fn try_lock(&self) -> Result<MutexGuard<T>> {
        if self.is_taken.compare_exchange(false, true, Ordering::SeqCst, Ordering::SeqCst).is_ok() {
            self.taker_line_num.store(Location::caller().line(), Ordering::SeqCst);
    ...
    pub fn lock(&self) -> MutexGuard<T> {
        ...
        panic!("Failed to lock Mutex at {}:{}, caller: {:?}, taker_line_num: {}", self.created_at_file, self.created_at_line, Location::caller(), self.taker_line_num.load(Ordering::SeqCst));
    }
