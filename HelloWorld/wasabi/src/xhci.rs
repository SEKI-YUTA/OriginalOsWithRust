...
const _: () = assert!(size_of::<InterruptRegisterSet>() == 0x20);

#[repr(C)]
pub struct RuntimeRegisters {
    mfindex: Volatile<u32>,
    rsvdz: [u32; 7],
    irs: [InterruptRegisterSet; 1024]
}
...
impl ScratchpadBuffers {
    fn alloc(
        ...
        for sb in table.iter_mut() {
            ...
            let buf = unsafe {
                slice::from_raw_parts(buf as *const u8, page_size)
            };
