...
impl PciXhciDriver {
    ...
    async fn run(xhc: Controller) -> Result<()> {
        ...
        if let Some(port) = connected_port {
            ...
            info!("slot {slot} is assinged for port {port}");
            let mut ctrl_ep_ring = Self::address_device(&xhc, port, slot).await?;
            info!("AddressDeviceCommand Succeeded");
            let device_descriptor = Self::request_device_descriptor(&xhc, slot, &mut ctrl_ep_ring)
            .await?;
            info!("Got a DeviceDescriptor: {device_descriptor:?}");
            let vid = device_descriptor.vender_id;
    ...
    async fn request_string_descriptor(
        ...
        xhc.request_descriptor(slot, ctrl_ep_ring, UsbDescriptorType::String, index, lang_id, buf.as_mut())
        .await?;
        Ok(String::from_utf8_lossy(&buf[2..]).to_string().replace('\0', ""))
    }
    async fn request_string_descriptor_zero(
        ...
        let buf = vec![0; 8];
        let mut buf = Box::into_pin(buf.into_boxed_slice());
        xhc.request_descriptor(slot, ctrl_ep_ring, UsbDescriptorType::String, 0, 0, buf.as_mut())
        .await?;
        Ok(buf.as_mut().as_ref().get_ref().to_vec())
    }
    async fn request_config_descriptor_and_rest(
        ...
        xhc.request_descriptor(slot, ctrl_ep_ring, UsbDescriptorType::Config, 0, 0, config_descriptor.as_mut().as_mut_slice())
        .await?;
        let buf = vec![0; config_descriptor.total_length()];
        let mut buf = Box::into_pin(buf.into_boxed_slice());
        xhc.request_descriptor(slot, ctrl_ep_ring, UsbDescriptorType::Config, 0, 0, buf.as_mut())
        .await?;
    ...
    fn alloc(cap_regs: &CapabilityRegisters, op_regs: &OperationalRegisters) -> Result<Self> {
        ...
        let table = ALLOCATOR.alloc_with_options(
            Layout::from_size_align(size_of::<usize>() * num_scratchpad_bufs, page_size)
                .map_err(|_| "cloud not allocate scratchpad buffer table")?,
        );
...
impl EndpointContext {
    ...
    fn new_control_endpoint(
        ...
        // 6.2.3 Software shell set Average TRB Length to '8' for control rndpoints
        Ok(ep)
    }
...
impl DeviceContext {
    ...
    fn set_last_valid_dci(&mut self, dci: usize) -> Result<()> {
        ...
        // For example, ((Context Entries+1) * 32 bytes) = Total bytes 
        // for this structure
        // 6.2.2.2
...
impl DeviceContextBaseAddressArray {
    ...
    fn set_output_context(
        ...
        // ...and set it in the acutla pointer array
        unsafe {
...
// Limiting the size of TrbRing to be equal or less than 4096 to avoid corssing 64kib boundaries. See Table 6-1 of xhci spec.
...
impl TrbRing {
    ...
    fn write_current(&mut self, trb: GenericTrbEntry) {
        self.write(self.current_index, trb)
            .expect("writing to the current idnex shell not fail")
    }
}
...
impl GenericTrbEntry {
    const CTRL_BIT_INTERUPT_ON_SHORT_PACKET: u32 = 1 << 2;
    const CTRL_BIT_INTERUPT_ON_COMPLETION: u32 = 1<< 5;
    ...
    fn cmd_result_ok(&self) -> Result<()> {
        if self.trb_type() != TrbType::CommandCompletionEvent as u32 {
            Err("Not as CommandCompletionEvent")
        } else if self.completion_code() != 1 {
            info!("Completion code was not Success. actual = {}",self.completion_code());
    ...
    fn transfer_result_ok(&self) -> Result<()> {
        if self.trb_type() != TrbType::TransferEvent as u32 {
            ...
            info!("Transfer failed. Actula CompletionCode = {}", self.completion_code());
            Err("CompletionCode was not Success")
        } else {
            Ok(())
        }
...
impl Default for CommandRing {
    fn default() -> Self {
        ...
        unsafe { this.ring.get_unchecked_mut() }
            ...
            .expect("faield to write a link trb");
        this
    }
}
...
// Interface ti access PORTSC registers
// [xhci]: 5.4.8 PORTSC
...
impl PortScEntry {
    ...
    pub fn port_speed(&self) -> UsbMode {
        ...
        // Default mapping is in table 7-13: Default USB Spped ID Mapping
...
// index 1-255: for device context s (index by a Slot ID)
    ...
    pub fn new_in<T: Sized>(buf: Pin<&mut [T]>) -> Self {
        Self {
            ...
            control: (TrbType::DataStage as u32) << 10
                ...
                | GenericTrbEntry::CTRL_BIT_INTERUPT_ON_COMPLETION
                | GenericTrbEntry::CTRL_BIT_INTERUPT_ON_SHORT_PACKET
        }
    }
}

// Status stage direction will be opposide of the data
// If there is no dat transfer, status direction should be "in"
