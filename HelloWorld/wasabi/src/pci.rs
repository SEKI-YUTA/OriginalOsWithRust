...
impl BusDeviceFunction {
    pub fn new(bus: usize, device: usize, function: usize) -> Result<Self> {
        ...
        {
            return Err("PCI bus device function of of range");
        } else {
            Ok(
                Self {
                    id: ((bus << SHIFT_BUS)
...
impl<T> ConfigRegisters<T> {
    fn read(ecm_base: *mut T, byte_offset: usize) -> Result<T> {
        if !(0..256).contains(&byte_offset) || byte_offset % size_of::<T>() != 0 {
            Err("PIC ConfigRegisters read out of range")
        } else {
            unsafe {
...
impl Pci {
    ...
    pub fn read_vendor_id_and_device_id(
        ...
        let vendero = self.read_register_u16(id, 0).ok()?;
        let device = self.read_register_u16(id, 2).ok()?;
        if vendero == 0xffff && device == 0xffff {
            None
        } else {
            Some(VendorDeviceId {
                vendor: vendero,
